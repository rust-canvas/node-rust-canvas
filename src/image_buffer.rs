...
pub fn image_buffer<'a, S: Scope<'a>>(o: Vec<u8>, scope: &'a mut S, width: u32, height: u32, format: ImageFormat, encoder_options: f32) -> JsResult<JsValue> {
  let mut dist = vec![];
  {
    let png_buffer = ImageBuffer::from_raw(width, height, o).unwrap();
    let dynamic_image = ImageRgba8(png_buffer);
    dynamic_image.save(&mut dist, format).unwrap();
  };
