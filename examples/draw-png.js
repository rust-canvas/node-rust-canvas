...
image.src = fs.readFileSync(join(process.cwd(), 'examples', 'image.png'))

ctx.drawImage(image, 0, 0).then(() => {
  ctx.lineWidth = 20
  ...
  canvas.toDataURL('image/png').then(base64 => {
    fs.writeFileSync(join(process.cwd(), 'examples', 'draw-png.png'), base64.split(',')[1], {
      encoding: 'base64'
    })
  })
})
